...
/**
 * @brief   Event records for the 48 GPIO lines.
 */
palevent_t _pal_events[48];
#endif
...
OSAL_IRQ_HANDLER(RP_IO_IRQ_BANK0_HANDLER) {
  ...
  for(int i=0; i < 4; ++i) {
    int line;
    uint32_t ints = io_bank0_hw->proc[RP_PAL_EVENT_CORE_AFFINITY].ints[i];

    io_bank0_hw->intr[i] = ints;
    line = i * 8;
    while (ints != 0U) {
      if (ints & 0x0FU) {
        _pal_isr_code(line);
...
void __pal_lld_init(void) {
  ...
  hal_lld_peripheral_unreset(RESETS_RESET_IO_BANK0_BITS);
  hal_lld_peripheral_unreset(RESETS_RESET_PADS_BANK0_BITS);
...
void _pal_lld_enablelineevent(ioline_t line, ioeventmode_t mode) {
  osalDbgAssert(
    !(io_bank0_hw->proc[RP_PAL_EVENT_CORE_AFFINITY].inte[line/8] & (0x0FU << (4U*(line & 0x07U)))),
    "channel in use");
  ...
  io_bank0_hw->proc[RP_PAL_EVENT_CORE_AFFINITY].inte[line/8] &= 
...
void _pal_lld_forcelineevent(ioline_t line) {
  ...
  io_bank0_hw->proc[RP_PAL_EVENT_CORE_AFFINITY].intf[line/8] &= ~force_mask;
  io_bank0_hw->proc[RP_PAL_EVENT_CORE_AFFINITY].intf[line/8] |= force_mask;
};
