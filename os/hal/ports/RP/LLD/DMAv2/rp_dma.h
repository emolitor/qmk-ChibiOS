 ...
 */
typedef struct {
  dma_hw_t              *dma;           /**< @brief Associated DMA.         */
  dma_channel_hw_t      *channel;       /**< @brief Associated DMA channel. */
...
__STATIC_INLINE bool dmaChannelIsBusyX(const rp_dma_channel_t *dmachp) {
  ...
  return (bool)((dmachp->channel->ctrl_trig & DMA_CH0_CTRL_TRIG_BUSY_BITS) != 0U);
}
...
__STATIC_INLINE void dmaChannelEnableInterruptX(const rp_dma_channel_t *dmachp) {
  ...
  if (sio_hw->cpuid == 0U) {
    dmachp->dma->inte0 |= dmachp->chnmask;
  }
  else {
    dmachp->dma->inte1 |= dmachp->chnmask;
  }
}
...
__STATIC_INLINE void dmaChannelSetCounterX(const rp_dma_channel_t *dmachp,
  ...
  dmachp->channel->transfer_count = n;
}
...
__STATIC_INLINE void dmaChannelSetModeX(const rp_dma_channel_t *dmachp,
  ...
  dmachp->channel->ctrl_trig = (mode & ~DMA_CH0_CTRL_TRIG_CHAIN_TO_BITS) |
                               (dmachp->chnidx << DMA_CH0_CTRL_TRIG_CHAIN_TO_LSB);
}
...
__STATIC_INLINE void dmaChannelAbortX(const rp_dma_channel_t *dmachp) {
  ...
  dmachp->dma->abort |= dmachp->chnmask;
  while ((dmachp->dma->abort & dmachp->chnmask) != 0U) {
  }
}
...
__STATIC_INLINE void dmaChannelEnableX(const rp_dma_channel_t *dmachp) {
  ...
  dmachp->channel->ctrl_trig |= DMA_CH0_CTRL_TRIG_EN_BITS;
}
